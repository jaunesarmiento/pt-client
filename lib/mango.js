...
Mango.prototype._initialize = function (options) {
  if (!options) {
    console.log("Error: missing paramter options");
    return;
  }
...
Mango.prototype._setStatus = function (options, cb) {
  ...
  if (!options) {
    console.log("Error: missing options parameter");
    return;
  }

  if (!options.storyId) {
    console.log("Error: missing option storyId");
    return;
  }
...
var mango = new Mango({ token: 'c0d4bc0f8ecade8f868ab1c72cccec12', projectId: '892984' });

mango.start('55340544', function (story) {
  console.log(story);
});
